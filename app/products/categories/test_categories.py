...
async def test_create_category(client, manager_token):
    ...
    resp = await client.post(url="/api/categories/", json=data, headers={"Authorization": f"Bearer {manager_token}"})
    assert resp.status_code == http.HTTPStatus.OK
...
async def test_update_category(client, category_factory, manager_token):
    ...
    resp = await client.patch(
        url=f"/api/categories/{category.id}", json=data, headers={"Authorization": f"Bearer {manager_token}"}
    )
    assert resp.status_code == http.HTTPStatus.OK
    assert resp.json()["name"] == data["name"]
...
async def test_delete_category(client, category_factory, manager_token):
    ...
    resp = await client.delete(
        url=f"/api/categories/{category.id}", headers={"Authorization": f"Bearer {manager_token}"}
    )
    assert resp.status_code == http.HTTPStatus.OK
